...
function ResourcePreview({ input }: { input: NodeGenerationInput }) {
    if (input.type === "image" && input.image) return <img src={imagePreviewUrl(input.image.dataUrl, 96)} alt="" className="size-9 rounded-md object-cover" />;
...
function createReferenceChip(input: NodeGenerationInput, theme: (typeof canvasThemes)[keyof typeof canvasThemes], onImagePreview: (url: string) => void) {
    ...
    if (input.type === "image" && input.image) {
        ...
        image.src = imagePreviewUrl(input.image.dataUrl, 96);
