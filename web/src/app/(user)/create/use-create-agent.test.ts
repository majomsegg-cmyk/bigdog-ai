...
describe("useCreateAgent submission retry", () => {
    it("keeps multiple Agent runs connected in one conversation", async () => {
        const source = await readFile(resolve(process.cwd(), "src/app/(user)/create/use-create-agent.ts"), "utf8");

        expect(source).toContain("const streamRefs = useRef(new Map<string, () => void>());");
        expect(source).toContain("streamRefs.current.set(run.id, stop)");
        expect(source).toContain("Promise.all(");
        expect(source).not.toContain("streamRef.current?.()");
        expect(source).not.toContain("sending || submittingRef.current");
    });
    ...
    it("keeps delayed run callbacks and controls scoped to the active conversation", async () => {
        ...
        expect(source.slice(cancelStart, controlStart)).toContain('controlCreativeAgentRun(runId, "cancel", expectedConversationId)');
    });
