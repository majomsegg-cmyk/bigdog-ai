...
export function useCreateAgent() {
    const streamRefs = useRef(new Map<string, () => void>());
    ...
    const syncSending = useCallback(() => {
        setSending(pendingSubmissionCountRef.current > 0 || streamRefs.current.size > 0);
    }, []);

    const stopWatching = useCallback(() => {
        for (const stop of streamRefs.current.values()) stop();
    ...
    const resetConversationView = useCallback((id?: string) => {
        ...
        setRunDetails({});
        setMessages([]);
        optimisticSequenceRef.current = 0;
    ...
    const refreshConversation = useCallback(async (id: string, generation = conversationGenerationRef.current) => {
        ...
        optimisticSequenceRef.current = Math.max(optimisticSequenceRef.current, nextMessages.reduce((max, item) => Math.max(max, item.sequence), 0) + 1);
    ...
    const ensureConversation = useCallback(
        async (generation = conversationGenerationRef.current) => {
            ...
            const creation =
                ...
                createCreativeConversation({ surface: "chat", source: "agent", title: "新对话" }).then((conversation) => {
    ...
    const watchRun = useCallback(
        (run: CreativeAgentRun, assistantMessageId: string, generation = conversationGenerationRef.current) => {
            ...
            streamRefs.current.get(run.id)?.();
            setActiveRunId(run.id);
            activeRunIdRef.current = run.id;
            setActiveRunStatus(run.status);
            const stop = watchCreativeAgentRun(run.id, {
                onProgress: (text) => {
                ...
                onTerminal: (status, text) => {
                    ...
                    streamRefs.current.delete(run.id);
                    if (activeRunIdRef.current === run.id) {
                        const nextRunId = Array.from(streamRefs.current.keys()).at(-1);
                        activeRunIdRef.current = nextRunId;
                        setActiveRunId(nextRunId);
                        setActiveRunStatus(nextRunId ? "running" : undefined);
                    }
                    syncSending();
            ...
            streamRefs.current.set(run.id, stop);
            syncSending();
            return true;
        },
        [isCurrentConversation, materializeProject, refreshAssets, refreshConversation, refreshConversations, syncSending, updateAssistant],
    );

    useEffect(() => {
        if (!conversationId || activeConversationRef.current !== conversationId) return;
        const running = messages.filter((item) => item.role === "assistant" && item.status === "running" && item.runId);
        if (!running.length) return;
        const generation = conversationGenerationRef.current;
        const expectedConversationId = conversationId;
        void Promise.all(
            running.map(async (message) => {
                if (!message.runId || streamRefs.current.has(message.runId)) return;
                const run = await getCreativeAgentRun(message.runId).catch(() => null);
                if (!run || !isCurrentConversation(expectedConversationId, generation) || run.conversationId !== expectedConversationId) return;
                watchRun(run, message.id, generation);
            }),
        );
    }, [conversationId, isCurrentConversation, messages, watchRun]);
    ...
    const executeSubmission = useCallback(
        ...
        [refreshConversations, syncSending, updateAssistant, watchRun],
    );
    ...
    const submit = useCallback(
        async (prompt: string, options?: CreateSubmitOptions) => {
            ...
            if (!executionPrompt || !content) return false;
            const generation = conversationGenerationRef.current;
            ...
            const now = Date.now();
            const sequence = Math.max(optimisticSequenceRef.current, messages.reduce((max, item) => Math.max(max, item.sequence), 0) + 1);
            optimisticSequenceRef.current = sequence + 2;
            const temporaryUserId = `message-${nanoid()}`;
            const temporaryAssistantId = `message-${nanoid()}`;
            let submittedConversationId = prepared.conversationId || submissionConversationId;
            if (!submittedConversationId) {
                try {
                    submittedConversationId = await ensureConversation(generation);
                } catch (error) {
                    pendingSubmissionCountRef.current = Math.max(0, pendingSubmissionCountRef.current - 1);
                    syncSending();
                    throw error;
            ...
            try {
                return await executeSubmission(snapshot);
            } finally {
                pendingSubmissionCountRef.current = Math.max(0, pendingSubmissionCountRef.current - 1);
                syncSending();
            }
        },
        [ensureConversation, executeSubmission, isCurrentConversation, materializeDraftAttachments, messages, selectedAssetIdsWithDrafts, syncSending],
    );

    const retrySubmission = useCallback(
        async (assistantMessageId: string) => {
            const snapshot = failedSubmissionsRef.current.get(assistantMessageId);
            if (!snapshot || !isCurrentConversation(snapshot.conversationId, snapshot.generation)) return false;
            pendingSubmissionCountRef.current += 1;
            syncSending();
        ...
        [executeSubmission, isCurrentConversation, syncSending, updateAssistant],
    );

    const cancel = useCallback(async () => {
        const expectedConversationId = activeConversationRef.current;
        const runId = activeRunIdRef.current;
        if (!runId || !expectedConversationId) return;
        await controlCreativeAgentRun(runId, "cancel", expectedConversationId);
    }, []);

    const control = useCallback(async (_action: "pause" | "resume") => undefined, []);
